...
export default function Page() {
  ...
  useEffect(() => {
    if (globaldata) {
      setData(Array.isArray(globaldata) ? globaldata : []);
    }
  }, [globaldata]);
  console.log(search)
  return (
    <div>
      ...
      {Data?( Data.map((ele) => {
        ...
        return (ele._id == search) &&
          <div key={Math.random()} className="py-14 px-4 md:px-6 2xl:px-20 2xl:container 2xl:mx-auto overflow-x-hidden">

            <div className="flex justify-start item-start space-y-2 flex-col">
              <h1 className="text-base dark:text-white sm:text-2xl lg:text-4xl  font-semibold leading-7 lg:leading-9  text-gray-600 ">Order: <span className='text-gray-600 font-normal'>{ele._id}</span></h1>
              <p className="text-base dark:text-gray-300 font-medium leading-6 text-gray-600">{new Date().getMonth()+1}/{new Date().getDate()}/{new Date().getFullYear()}</p>
            </div>
            <div className="mt-10 flex flex-col xl:flex-row jusitfy-center items-stretch w-full xl:space-x-8 space-y-4 md:space-y-6 xl:space-y-0">
              <div className="flex flex-col justify-start items-start w-full space-y-4 md:space-y-6 xl:space-y-8">
                <div className="flex flex-col justify-start items-start dark:bg-gray-800 bg-gray-50 px-4 py-4 md:py-6 md:p-6 xl:p-8 w-full">
                  ...
                  {(Object.keys(ele.productName)).map((element)=>{return <div key={Math.random()} className="mt-4 md:mt-6 flex flex-col md:flex-row justify-start items-start md:items-center md:space-x-6 xl:space-x-8 w-full">
                    <div className="pb-4 md:pb-8 w-40 md:w-40">
                      <img className="w-full hidden md:block" src={ele.productName[element].image} alt="dress" />
                      <img className="w-full md:hidden" src={ele.productName[element].image} alt="dress" /> 
                    </div>
                    <div className="border-b border-gray-200 md:flex-row flex-col flex justify-between items-start w-full pb-8 space-y-4 md:space-y-0">
                      <div className="w-full flex flex-col justify-start items-start space-y-8">
                        <h3 className="text-xl dark:text-white xl:text-2xl font-semibold leading-6 text-gray-800">{ele.productName[element].name}</h3>
                        <div className="flex justify-start items-start flex-col space-y-2">
                          ...
                          <p className="text-sm dark:text-white leading-none text-gray-800"><span className="dark:text-gray-400 text-gray-300">Color: </span> Light</p>
                        </div>
                      </div>
                      <div className="flex justify-between space-x-8 items-start w-full">
                        <p className="text-base dark:text-white xl:text-lg leading-6">${ele.productName[element].price} </p>
                        <p className="text-base dark:text-white xl:text-lg leading-6 text-gray-800">{ele.productName[element].quantity}</p>
                  ...
                  <div className="flex flex-col px-4 py-6 md:p-6 xl:p-8 w-full bg-gray-50 dark:bg-gray-800 space-y-6">
                    ...
                    <div className="flex justify-center items-center w-full space-y-4 flex-col border-gray-200 border-b pb-4">
                      <div className="flex justify-between w-full">
                        ...
                        <p className="text-base dark:text-gray-300 leading-4 text-gray-600">{ele.price}</p>
                      </div>
                    ...
                    <div className="flex justify-between items-center w-full">
                      ...
                      <p className="text-base dark:text-gray-300 font-semibold leading-4 text-gray-600">${ele.price}</p>
                    </div>
                  </div>
                ...
                <div className="flex flex-col md:flex-row xl:flex-col justify-start items-stretch h-full w-full md:space-x-6 lg:space-x-8 xl:space-x-0">
                  <div className="flex flex-col justify-start items-start flex-shrink-0">
                    <div className="flex justify-center w-full md:justify-start items-center space-x-4 py-8 border-b border-gray-200">
                      ...
                      <div className="flex justify-start items-start flex-col space-y-2">
                        <p className="flex items-center justify-center text-base dark:text-white font-semibold leading-4 text-left text-gray-800">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M19 5H5C3.89543 5 3 5.89543 3 7V17C3 18.1046 3.89543 19 5 19H19C20.1046 19 21 18.1046 21 17V7C21 5.89543 20.1046 5 19 5Z" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" />
                        <path d="M3 7L12 13L21 7" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" />
                      </svg>{Loggedin? <p className=''>{Loggedin}</p>:<></>}</p>
